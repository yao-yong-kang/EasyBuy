...
def car1(request):
    ...
    if username:
        ...
        return render(request, 'goods_car/BuyCar.html', {'product_list': product_list,"count":count,'username':username})
    else:
...
def car2(request):
    ...
    add = Address.objects.get(isDefault='1', id=user.id)  # 获取当前用户的默认地址
    total = 0  # 消费
    ...
    if request.method == 'POST' and request.POST.get('city'):
        print(1)
        # list[-1] - user.score / 100
        print('city是',request.POST.get('city'))
        return redirect('goodcar3')
        # return render(request, 'goods_car/BuyCar_Two.html',
        #                       {'product_list': product_list, 'total': total, "user": user, 'add': add})
...
def car3(request):
    ...
    if list[-1] >= user.money:    #余额不知时
        ...
        if add:
            ...
            return render(request, 'goods_car/BuyCar_Two.html',{'list': list[-1],"msg":"您还没有收货地址"})
        for i in product_list:
        ...
        return render(request, 'goods_car/BuyCar_Three.html',{'order':order1,'order_detail':order_detail1,'list':list[-1]})

def car4(request):
    return render(request,'goods_car/BuyCar_Four.html')
